...
def fwhm(sigma_g, gamma_l):
    ...
    return np.power( np.power(2.*np.sqrt(2.*np.log(2.))*sigma_g,5)*np.power(gamma_l,0) + 
                     ...
                     np.power(2.*np.sqrt(2.*np.log(2.))*sigma_g,2)*np.power(gamma_l,3)*4.47163 + 
                     np.power(2.*np.sqrt(2.*np.log(2.))*sigma_g,1)*np.power(gamma_l,4)*0.07842 + 
                     np.power(2.*np.sqrt(2.*np.log(2.))*sigma_g,0)*np.power(gamma_l,5), 0.2 )

def mix(gamma_l, f):
    """Calculates the mixing parameter 'm' for mixing Gaussian and Lorentzian
    components together.
    
    Magic numbers come from P. Thompson, D.E. Cox, J.B. Hastings, J. Appl. Cryst. 1987, 20, 79.
...
def d2Voigt(prof,x):
    ...
    #Calculate the mixing parameter and its first and second PARTIAL derivatives:
    m = mix(gamma_l, f)
    ...
    #Calculate the normalized Gaussian and Lorentzian profiles at the location of interest, as well as their derivatives
    g = (4./f)*np.sqrt(np.log(2.)/np.pi)*np.exp(-4.*np.power((x-lambda_0)/f,2))
    ...
    #Calculate a few derivatives w.r.t. the parameter q = gamma/f (because I apparently can't do math)
    ...
    #Calculate first derivatives of the Voigt profile at the location of interest
    dV_dlambda = I*(m*dl_dlambda + (1.-m)*dg_dlambda)
    dV_dsigma = I*df_dsigma*(m*dl_df+(l-g)*dm_df+(1.-m)*dg_df)
    dV_dgamma = I*(m*dl_df*df_dgamma+(1.-m)*dg_df*df_dgamma+(l-g)*dm_dq*dq_dgamma)
        
    #Calculate the second derivatives of the Voigt profile at the location of interest
...
def FitVoigt(x, y, dy, fix = [None,None,None,None,None]):
    ...
    Returns:
    fit -> Fitted values for the 5 parameters which were fitted (including fixed ones)
    unc -> Uncertainties in the 5 parameters. Uncertainties in fixed parameters are zero.
    redchi2 -> Reduced chi^2 statistic chi^2/n, where n is the number of fitted parameters
...
def fit_ha_and_n2(x, y, dy):
    ...
    x=x[mask][args]
    y=y[mask][args]
    if (not dy is None): dy=dy[mask][args]
    guess = np.zeros(8)
