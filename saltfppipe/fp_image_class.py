...
class FPImage:
    ...
    update -> Bool. Overwrite changes to original file? Default = False.
    ...
    def __init__(self, filename, update=False):
        ...
        try:
            self.wave = self.openimage[4].data
        except:
            self.wave = None
        ...
        if "Etalon 1" in self.header.get("et-state"):
            self.state = "mr"
        elif "Etalon 2" in self.header.get("et-state"):
            self.state = "lr"
        else:
            self.state = "mr"
    ...
    def close(self):
        ...
        if self.update:
            self.writeto(self.filename, clobber=True)
    ...
    def writeto(self, outfilepath, clobber=False):
        ...
        update_header_kw(self.header, "exptime", self.exptime)
        ...
        if self.wave is not None:
            self.openimage[4].data = self.wave
    ...
    def skybackground(self):
        ...
        data = data[np.logical_and(data > lowval,
                                   data < highval)]
