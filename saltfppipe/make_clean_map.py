...
def make_clean_map(cubedir, clobber=False):
    ...
    maxsigmarat = float(raw_input("Enter maximum dsigma/sigma ratio (or 0 for no cut): "))
    maxintyrat = float(raw_input("Enter maximum dinty/inty ratio (or 0 for no cut): "))
    ...
    if maxchi2!=0: mask = np.logical_and(mask, chi2image[0].data<maxchi2)
    if maxsigmarat!=0: mask = np.logical_and(mask, normalized_dsig<maxsigmarat)
    if maxintyrat!=0: mask = np.logical_and(mask, normalized_dinty<maxintyrat)
