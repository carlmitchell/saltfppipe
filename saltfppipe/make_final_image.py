...
def convolve_variance(vararray, badpixarray, kern):
    ...
    running_var_sum[mask] = (running_var_sum[mask] /
                             running_wgt_sum[mask]*np.sum(kern**2))
...
def convolve_wave(wavearray, intyarray, badpixarray, kern):
    ...
    running_wavsum = np.zeros_like(wavearray)
    running_wgtsum = np.zeros_like(wavearray)
    ...
    # how much of the kernel we ended up using
    running_wavsum[mask] = running_wavsum[mask]/running_wgtsum[mask]
    running_wavsum[np.logical_not(mask)] = wavearray[np.logical_not(mask)]

    return running_wavsum
...
def convolve_inty(intyarray, badpixarray, kern):
    ...
    running_intsum = np.zeros_like(intyarray)
    running_wgtsum = np.zeros_like(intyarray)
    ...
    for y in range(2*ksize+1):
        for x in range(2*ksize+1):
            ylim = ysize-2*ksize-1+y
            xlim = xsize-2*ksize-1+x
            running_intsum[ksize:ysize-ksize-1,
                           ksize:xsize-ksize-1] += (kern[y, x] *
                                                    intyarray[y:ylim, x:xlim] *
                                                    mask[y:ylim, x:xlim])
            running_wgtsum[ksize:ysize-ksize-1,
    ...
    # how much of the kernel we ended up using
    running_intsum[mask] = running_intsum[mask]/running_wgtsum[mask]
    running_intsum[np.logical_not(mask)] = 0

    return running_intsum
